...
### 1. 核心源码Patch（推荐使用）
```
📄 nginx-1.28-proxy-protocol-v2-enhanced.patch
```
...
- **应用方法**: `patch -p1 < nginx-1.28-proxy-protocol-v2-enhanced.patch`
...
cp /path/to/nginx-1.28-proxy-protocol-v2-enhanced.patch .

# 应用patch（增强Proxy Protocol v2支持）
patch -p1 < nginx-1.28-proxy-protocol-v2-enhanced.patch
...
server {
    location / {
        proxy_pass http://rtt_aware_backend;
...
### 常见问题
1. **编译错误**: 确保应用了正确的patch
2. **模块未加载**: 检查--add-module路径
3. **RTT值为空**: 检查proxy_protocol配置
4. **变量不工作**: 确认启用了相应的配置指令
...
# 检查模块加载
/usr/local/nginx/sbin/nginx -V 2>&1 | grep tcp_rtt

# 验证配置
/usr/local/nginx/sbin/nginx -T

# 测试TLV功能
echo "PROXY TCP4 127.0.0.1 12345 192.168.1.1 80\r\n" | nc localhost 9000
```
...
2. **模块实现**: 独立的Stream和HTTP模块，实现RTT计算、传递和暴露
3. **标准兼容**: 严格遵循Proxy Protocol v2规范和NGINX模块开发标准
4. **生产就绪**: 包含完整的错误处理、性能优化和监控支持
5. **文档齐全**: 提供中英文详细文档、配置示例和部署指南

**核心patch可直接应用到nginx-1.28.0源码，模块可独立编译使用，为负载均衡优化、性能监控和智能路由提供强大的网络性能分析能力。**